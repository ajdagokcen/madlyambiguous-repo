  ...
  var db;
	...
	wn.Synset.prototype = {
			  ...
			  getAttributes: function(callback){
				  var promise = _findAttributesArray(this.synsetid).map(_appendLemmas).map(function(item){
					  return new wn.Synset(item);
				  });
				  return promise.nodeify(callback);
			  },
	...
	function _findAttributesArray(synsetid){
		var arr;
		if (!wn.preload){
			var query = "SELECT synset2id FROM semlinks WHERE synset1id = $synset1id AND linkid = 60";
			arr =  db.allAsync(query,{
				$synset1id: synsetid
			});
		} else {
			arr = wn.SEMLINKS.filter(function(e){
				return e.synset1id === synsetid && e.linkid === 60;
			});
		}
		var ret = arr.map(function(d){
			var obj = {};
